...
rc = {"text.usetex": True, "font.family": "serif", "font.weight": "bold", "axes.labelweight": "bold",
          "font.serif": ["Palatino"], "xtick.labelsize": font_size, 'figure.figsize': (8, 8),
          "ytick.labelsize": font_size, 'axes.grid': True, 'axes.facecolor': 'white',
          'grid.linestyle': '--', 'grid.linewidth': 3, 'lines.linewidth': 3, "axes.linewidth":3,
          'axes.axisbelow': True}
...
#  [    0,    19,   241]])
#
# xgb_Early01 =  np.array([[1943,	121,	11],
# [9,	155,	16],
# [10,	16,	234]])
#
#
...
def plot_cm_from_csv(csv_path, title, save_path=None):
    ...
    # Normalize row-wise (true label normalization)
    ...
    sns.heatmap(
        ...
        linecolor="black",
        linewidths=3,
        annot_kws={"size": 36}, cbar=False,
        cmap="Blues"
    )

    plt.title(title + " (normalized)", fontsize =26)
    plt.axhline(y=0, color='k',linewidth=6)
    plt.axhline(y=3, color='k',linewidth=6)
    plt.axvline(x=0, color='k',linewidth=6)
    plt.axvline(x=3, color='k',linewidth=6)
    plt.tick_params(labelsize=24)
    plt.xlabel("True", fontsize=28)
    plt.ylabel("Predicted", fontsize=28)
    plt.tight_layout()
...
plot_cm_from_csv(
    csv_path="models_out_balanced/cm_RandomForest_early1_only.csv",
    title="RF - Early_1s",
    save_path="Figures/figures_paper/cm_RF_early1_only.png"
)
